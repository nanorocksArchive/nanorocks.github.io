...
@section('post_body')
    ...
    @markdown
    ### Intro
    There are a ton of topics and posts on the internet about Dependency injection and DI containers. My goal is to make it understandable, not confusing and be pragmatic on explaining.
    ...
    <ul>
        <li>A dependency is required and class can't work without it. By using constructor injection. we make sure all its required dependencies are passed.</li>
        <li>Since constructor is called only at the time of instantiating a class, we can make sure that it’s dependencies can’t be changed during the lifetime of the object.</li>
    </ul>
    ...
    <p>A class might have many dependencies. It isn't all that easy to manage all those dependencies because you need to KNOW which dependencies are required by a certain class and HOW they need to be instantiated.  It becomes harder to manage all of those manually and you could easily and mistakenly create more than one instances of dependencies throughout your code which would result in high memory usage.</p>
    <p>
    ...
    <br>
    <h2>DI Container</h2>
    <p>A Dependency Injection Container is something that handles dependencies for your classes automatically. For example modern web frameworks in PHP world like Simfony or Laravel have components that have dependencies on other classes. How do they manage all of those dependencies ? Yes they use some sort of Dependency Injection Container.</p>
    <p>There are quite some dependency injection containers on the internet that can be used for this purpose or you can also write your own. Each container might have a bit of different syntax but they perform the same thing under the hood.</p>

    <br>
    <h2>Conclusion</h2>
    <p>In a real time project It’s better to use Dependencies Injection container that will automatically take care of the dependencies you use. Also, try to avoid hard-coded dependencies they don’t belong here. </p>
@stop